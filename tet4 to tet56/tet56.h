 ...
 */
typedef struct edge{
	int node_id[4];   /**< Node IDs unique to each node in edge */
	double x[4];      /**< X coordinates of nodes in edge */
	double y[4];	  /**< Y coordinates of nodes in edge */
	double z[4];	  /**< Z coordinates of nodes in edge */
 ...
 */
typedef struct face {
	int node_id[6];   /**< Node IDs unique to each node in face */
	double x[6];	  /**< X coordinates of nodes in face */
	double y[6];      /**< Y coordinates of nodes in face */
	double z[6];	  /**< Z coordinates of nodes in face */
...
typedef struct internal {
	int node_id[4];		/**< Node IDs unique to each node in internal */
	double x[4];		/**< X coordinates of nodes in internal */
	double y[4]; 	    /**< Y coordinates of nodes in internal */
	double z[4];		/**< Z coordinates of nodes in internal */
} internal_t;

/**
* Method to be called from main function that does all reading, constructing, and printing of new elements
* NOTE: this implementation was not completed 
* @author Dan Gross
* @date 30 July 2017
...
* @return char* representation of Internal Node IDs for use in new element
*/
...
/**
* Math function that computes the distance one fifth of the way between a and b
* @author Dan Gross
* @date 30 July 2017
* @param a First arg
* @param b Second arg
* @return The 'fifth' distance from a to b
*/ 
double fifths( double a, double b );
/**
* Math function that computes the distance two fifths of the way between a and b
* @author Dan Gross
* @date 30 July 2017
* @param a First arg
* @param b Second arg
* @return The 'two-fifths' distance from a to b
*/ 
