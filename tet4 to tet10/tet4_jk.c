...
void readTET4( const char* msh_file, int* num_nodes, int* num_TET4, NODE** mynodes, EL_TET4** myTET4 )
{
	...
	while (fgets(buff, 256, msh) != 0) {
		...
		// Reading all element connectivity and then saving only TET4 element connectivity
...
{
	double begin = omp_get_wtime();
	printf("\tStart constructing edge objects from TET4 elements:\n");

	int num_all_edges = *num_TET4 * MAX_EDGES;			// The maximum number of edges including repeated edges
	ED_TET4* tmp_edges = (ED_TET4*) malloc(num_all_edges * sizeof(ED_TET4));
	ED_TET4* unique_edges;
	// 0-based CSR format
	...
	#pragma omp parallel default(shared) private(here)
	{
		...
		for (int i=0; i<*num_TET4; i++) {
			// The 1st edge		
			tmp_edges[0+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[0];
			tmp_edges[0+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[1];
			// The 2nd edge
			tmp_edges[1+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[1];
			tmp_edges[1+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[2];
			// The 3rd edge
			tmp_edges[2+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[2];
			tmp_edges[2+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[0];
			// The 4th edge
			tmp_edges[3+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[0];
			tmp_edges[3+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[3];
			// The 5th edge
			tmp_edges[4+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[1];
			tmp_edges[4+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[3];
			// The 6th edge
			tmp_edges[5+i*MAX_EDGES].nodeID[0] = (*myTET4)[i].nodeID[2];
			tmp_edges[5+i*MAX_EDGES].nodeID[1] = (*myTET4)[i].nodeID[3];
		}

		#pragma omp for 					// Copying tmp_edges to ref_edges and then sorting nodeID of tmp_edges
		for (int i=0; i<*num_TET4; i++) {
			for(int j=0; j<MAX_EDGES; j++) {
				sort_edge_node(&(tmp_edges[ j+i*MAX_EDGES ]), &((*myTET4)[i].edgeDIR[j]) );
			}
		}	
		...
		#pragma omp single 					
		{
			...
			*num_edges = link_edges_ptr_unique[*num_nodes];
			unique_edges = (ED_TET4*) malloc( *num_edges * sizeof(ED_TET4));
		...
		#pragma omp for  					// Filling *myTET4.edgeID
		for (int i=0;i<*num_TET4;i++) {
			for (int j=0; j<MAX_EDGES; j++) {
